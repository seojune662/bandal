...
설치 시 Bandal은 폴더 내용을 사용자 데이터 디렉터리로 복사합니다. 원본 개발 폴더를 수정해도 설치본은 자동으로 바뀌지 않습니다. 같은 플러그인을 다시 설치해 갱신하세요.

동작 예시는 [`examples/plugins/word-count`](../examples/plugins/word-count/README.md)에 있습니다.

## 위협 모델

신뢰 경계는 `node:vm` 컨텍스트가 아니라 별도의 플러그인 호스트 프로세스입니다. `vm`은 플러그인마다 깨끗하고 제한된 전역을 제공하는 실행 장치일 뿐, 단독 보안 샌드박스로 간주하지 않습니다. 호스트는 자격증명 저장소, 애플리케이션 DB, 메인 프로세스 객체, Electron API에 접근할 권한이나 참조를 받지 않습니다.
