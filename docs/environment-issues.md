...
### 스왑이 10GB를 먹고 있다 (2026-08-06 00:30 측정)

```
vm.swapusage: total = 10240M  used = 9232M
Pageouts: 3,417,438
```

에이전트 여러 개가 `tsc`/`vitest`/`esbuild`를 동시에 돌리면서 메모리 압박이 커졌고,
macOS가 스왑 파일을 10GB까지 키웠다. 스왑 파일은 디스크에 있으므로 **여유 공간을 그대로 먹는다.**
게다가 macOS는 스왑을 적극적으로 줄이지 않는다.

- **재부팅하면 스왑이 초기화되어 즉시 ~10GB가 돌아온다.** 지금 가장 값싼 회복 수단이다.
- 근본 대책은 동시 실행 에이전트를 1개로 유지하는 것(§4.1).

### ⚠️ `pnpm store prune`은 역효과였다

공간 확보용으로 실행했더니 ~800MB가 반환됐지만, 이후 `pnpm install` 한 번에
**679개 패키지를 다시 내려받았다**(오프라인 메타데이터 캐시가 비워졌기 때문).
순증이 거의 없거나 오히려 손해다. **다시 하지 말 것.**
