...
완성된 설치 파일은 Actions 아티팩트에 3일간 보관하고, 별도 게시 작업이 릴리스에
업로드한다. 파일 전송이 실패하면 실패한 게시 작업만 재실행하면 되며 서명·공증을
반복할 필요가 없다. 업로드는 요청별 시간 제한과 최대 4회 재시도를 적용한다.

`scripts/upload-release-assets.mjs`는 설치 파일을 스트리밍으로 전송하고 크기와
SHA-256으로 서버의 저장 결과를 확인한다. 이미 일치하는 파일은 재전송하지 않는다.
목록 조회·삭제 오류와 불완전한 JSON 응답도 재시도하며, 응답만 유실된 업로드는
서버 상태를 다시 확인해 완료된 파일을 보존한다. 게시 도구는 워크플로의 커밋에서
가져오므로 이전 태그의 설치 파일을 그대로 두고 배포 절차만 보완할 수 있다.
