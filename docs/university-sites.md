...
5. **비밀번호는 이렇게만 다룬다.** (옛 규칙 "저장·자동입력을 만들지 마라"는 철회했다 —
   기능은 이미 구현·출시됐고, 근거 중 "보안 플러그인 쓰는 곳에선 어차피 안 된다"는
   전역 사실이 아니라 사이트별 사실이었다. `loginFiller` 는 평범한
   `input[type=password]` 를 요구하므로 nProtect 페이지에서는 `{filled:false}` 로
   그냥 물러난다. 문서를 그대로 두면 코드가 문서를 반박하는 상태가 되어, 다음 사람이
   어느 규칙이 살아 있는지 알 수 없다.)
   - 저장된 비밀번호는 OS 키체인 암호화 블롭 안에만 있고 평문 폴백이 없다.
     정확한 https 오리진에만 묶인다.
   - **채우기와 제출은 절대 한 번의 승인으로 묶이지 않는다.** `autoSubmit` 기본값은
     off 이고 사이트별 opt-in 이다.
   - **에이전트는 비밀번호를 볼 수 없다.** `browser_use_saved_login` 은 탭만 받고
     `{filled}` 만 돌려준다 — 요청할 수단 자체가 없다.
   - 보안 플러그인이 켜진 페이지에서는 자동입력을 시도하지 않고 실패를 그대로 보고한다.
   - **로그인 제출은 언제나 학생의 승인을 거친다.** `browser_submit` 은 매번 묻고
     그 승인은 기억되지 않는다.
...
10. **`setWindowOpenHandler` 의 팝업→탭 포워딩을 SSO 도메인에 그대로 적용하지 마라.**
    ...
    **구현됨 (v0.17 예정, `isSameSiteAcademicPopup`).** 같은 대학 안에서의 팝업만
    진짜 창으로 허용한다. 판정 단위는 `ac.kr` 을 공개 접미로 보는 등록가능 도메인
    (`inha.ac.kr`)이고 **포트는 비교하지 않는다** — 인하대 IdP 는 `:8443`, 포털은
    `:443` 인데 같은 기관이다. 허용한 창도 그냥 두지 않는다: 같은 파티션·sandbox·
    contextIsolation 을 상속하고, `did-create-window` 에서 `will-navigate` /
    `will-redirect` 가드를 다시 걸며, **그 창은 다시 팝업을 열 수 없다**(예외는 한
    겹이면 충분하다). 대학이 다르거나 http(s) 가 아니면 종전대로 탭으로 포워딩한다.
