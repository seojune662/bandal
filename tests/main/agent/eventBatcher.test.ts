...
describe('eventBatcher', () => {
  ...
  test('debounces deltas by 40ms and coalesces same-block text', () => {
    batcher.push(COURSE, textDelta('b1', 'Hel'))
    batcher.push(COURSE, textDelta('b1', 'lo '))
    batcher.push(COURSE, textDelta('b1', 'world'))
    ...
    expect(sent[0]).toMatchObject({ courseId: COURSE, seq: 1 })
    expect(sent[0]!.events).toEqual([textDelta('b1', 'Hello world')])
  })

  test('does not merge deltas across different blocks', () => {
    batcher.push(COURSE, textDelta('b1', 'a'))
    batcher.push(COURSE, textDelta('b2', 'b'))
  ...
  test('continuous deltas flush at the 250ms max wait', () => {
    ...
    for (let push = 0; push < 9; push += 1) {
      batcher.push(COURSE, textDelta('b1', 'x'))
  ...
  test('non-delta events flush the pending frame immediately', () => {
    batcher.push(COURSE, textDelta('b1', 'partial'))
    batcher.push(COURSE, {
      type: 'turn-complete',
      stopReason: 'success'
    })
  ...
  test('seq increases monotonically per course', () => {
    batcher.push(COURSE, { type: 'turn-complete', stopReason: 'success' })
    batcher.push(COURSE, { type: 'turn-complete', stopReason: 'success' })
    batcher.push('course-2', { type: 'turn-complete', stopReason: 'success' })
    expect(sent.map((batch) => [batch.courseId, batch.seq])).toEqual([
      [COURSE, 1],
      [COURSE, 2],
      ['course-2', 1]
    ])
  ...
  test('flush() sends pending deltas without waiting', () => {
    batcher.push(COURSE, textDelta('b1', 'now'))
    batcher.flush(COURSE)
    expect(sent).toHaveLength(1)
